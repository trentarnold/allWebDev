...
class Game {
  ...
  setCardHtml(nextCard, hidden = false) {
    if (hidden) {
      return $(
        `<div class='card hidden' data-value='${nextCard.value} ${nextCard.suit}' style='color:${nextCard.color}'>${nextCard.suit}</div>`
      );
    }
    return $(
      `<div class='card' data-value='${nextCard.value} ${nextCard.suit}' style='color:${nextCard.color}'>${nextCard.suit}</div>`
    );
  }
  checkBlackJack() {
  ...
  dealerPlay() {
    if (
      !this.includesAcesAtEleven(this.dealerHand) &&
      this.dealerValueTotaled >= 17
    ) {
      this.checkWinner();
      return;
    }
  ...
  checkAceValue(array, playerInString) {
    ...
    if (
      (playerInString === "Player" && this.playerValueTotaled < 22) ||
      (playerInString === "Dealer" && this.dealerValueTotaled < 22)
    )
      return;
  ...
  checkWinner() {
    if (this.dealerValueTotaled === this.playerValueTotaled) {
      ...
      this.winner =
        this.dealerValueTotaled > this.playerValueTotaled
          ? `Dealer Wins`
          : "Player Wins!";
      this.declareWinner();
    }
  ...
  updateTotals(firstCardHidden = true) {
    ...
    if (firstCardHidden) {
      $(".dealer-total").text(
        `Dealer showing: ${this.dealerHand[1].trueValue}`
      );
    } else {
      $(".dealer-total").text(`Dealer total: ${this.dealerValueTotaled}`);
    }
